...
$result = mysqli_query($conn,"SELECT  st.particular, st.bill, st.open, st.iqty, st.bal, storeitem.rate, storeitem.amount, storeitem.firm from st INNER JOIN storeitem on st.pid=storeitem.pid where (storeitem.date='$dte' and st.type='deadstock') order by particular");
if (!$result)
