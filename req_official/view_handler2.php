...
$result = mysqli_query($conn,"SELECT  st.particular, st.bill, st.open, st.iqty, st.bal, st.d_o_i, st.utd, st.remarks, storeitem.date, storeitem.rate, storeitem.amount, storeitem.firm from st INNER JOIN storeitem on st.pid=storeitem.pid where (st.d_o_i='$dte' and st.type='stock') order by particular");
if (!$result)
die("Could not retreive data" . mysql_error());
 ...
 {?>
  <div Style="font-size:20px; color:blue;"><b>Date Wise Accounting Report</b></div><br>
	 ...
	 <?php
echo "<table border='3'cellpadding='5' cellspacing='5'><tr >";
...
<div class="flash" style="font-size:20px;padding-top:20px; min-height:100px; font-weight: bold;">
	<?php echo "No Particulars Issued !";
}
