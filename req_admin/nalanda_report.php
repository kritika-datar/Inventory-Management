...
<?php
    ...
    { $s=$row['name'];}
	$query="SELECT  st.particular, st.bill, st.open, st.iqty, st.bal, st.d_o_i, st.utd, st.remarks, storeitem.date, storeitem.rate, storeitem.amount, storeitem.firm from st INNER JOIN storeitem on st.pid=storeitem.pid where (st.d_o_i BETWEEN '$fdate' AND '$tdate') AND (st.utd='$s') AND (st.type='stock') order by d_o_i";
...
?><br><br>
	<a href="rnt_list.php" style="color:blue; text-decoration:none;font-size: 18px; float: right;margin-right:20px;"><< <b>Back</b></a>
